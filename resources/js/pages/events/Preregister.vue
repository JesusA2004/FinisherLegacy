...
<template>
    ...
    <section class="mx-auto max-w-lg px-4 py-16 sm:px-6 lg:px-8">
        <template v-if="!edition || !isOpen">
            <div
                class="mx-auto flex size-16 items-center justify-center rounded-full border border-fl-gold/30 bg-fl-graphite/60 text-fl-gold"
            >
                <Clock class="size-7" />
            </div>
        ...
        <Reveal v-else>
            <div class="mb-8 text-center">
                <p
                    class="text-xs font-semibold tracking-widest text-fl-gold uppercase"
                >
                    Prerregistro
                </p>
                ...
                <p class="mt-1 text-sm text-white/50">{{ edition.name }}</p>
            </div>
