...
function resolve(conflict: Conflict, resolution: 'link_existing' | 'create_new' | 'ignore') {
    ...
    router.post(
        ...
        { preserveScroll: true, onFinish: () => pendingIds.value.delete(conflict.id) },
    );
}
...
<template>
    ...
    <div class="p-4 md:p-8">
        ...
        <p class="mb-6 text-sm text-white/50">
            ...
            nunca se fusiona por nombre solamente. La confianza es una ayuda,
            no una certeza: revísalo antes de decidir.
            <HelpPopover
            ...
            >
                <div class="mb-3 flex items-center justify-between">
                    <div class="flex items-center gap-2 text-xs text-white/40">
                        <span>{{ conflict.event ?? conflict.source_type }}</span>
                        <span>·</span>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div class="rounded-lg border border-white/10 bg-fl-black/40 p-3">
                        <p class="mb-1 text-xs text-white/30 uppercase">
                            Datos importados
                        </p>
                        <p class="text-white">
                            {{ conflict.incoming.full_name ?? `${conflict.incoming.first_name ?? ''} ${conflict.incoming.last_name ?? ''}` }}
                        </p>
                        <p class="text-xs text-white/50">
                            {{ conflict.incoming.email ?? 'Sin email' }} ·
                            {{ conflict.incoming.birth_date ?? 'Sin fecha de nacimiento' }}
                        </p>
                    </div>
                    <div class="rounded-lg border border-white/10 bg-fl-black/40 p-3">
                        <p class="mb-1 text-xs text-white/30 uppercase">
                        ...
                        <template v-if="conflict.candidate">
                            <p class="text-white">{{ conflict.candidate.full_name }}</p>
                            <p class="text-xs text-white/50">
                                {{ conflict.candidate.email ?? 'Sin email' }} ·
                                {{ conflict.candidate.birth_date ?? 'Sin fecha de nacimiento' }}
                            </p>
                        </template>
                        <p v-else class="text-sm text-white/30">Sin candidato específico.</p>
                    </div>
                </div>
                ...
                <div class="mt-3 flex flex-wrap gap-2">
                    <Button
                        ...
                        :disabled="!conflict.candidate || pendingIds.has(conflict.id)"
